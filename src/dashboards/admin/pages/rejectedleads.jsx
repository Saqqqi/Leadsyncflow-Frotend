...
export default function RejectedLeads() {
    ...
    return (
        <div className="p-4 md:p-6 space-y-4 max-w-[1400px] mx-auto animate-fadeIn min-h-screen">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative z-10">
                    <div className="flex flex-wrap items-center gap-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col">
                                <h1 className="text-3xl font-black tracking-tight text-[var(--text-primary)] whitespace-nowrap leading-tight">
                                    Audit <span className="text-emerald-500">Pipeline</span>
                                </h1>
                                <p className="text-[13px] font-bold text-[var(--text-secondary)] opacity-70 italic tracking-tight hidden sm:block">
                                    Systematic review and decisioning of node rejection requests
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                            <div className="flex flex-col">
                                <span className="text-[8px] font-black uppercase tracking-[0.2em] text-[var(--text-tertiary)] opacity-60">Nodes Waiting</span>
                                <div className="flex items-baseline gap-1">
                                    <span className="text-2xl font-black text-[var(--text-primary)] tabular-nums">{leads.length}</span>
                                    <span className="text-[10px] font-bold text-emerald-500/60 uppercase">Tickets</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center bg-[var(--bg-tertiary)]/40 border border-[var(--border-primary)]/40 rounded-full pl-1 pr-3 py-1 gap-3 hover:border-emerald-500/40 transition-all group/sync-badge">
                            ...
                            <div className="flex flex-col leading-none">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[7px] font-black uppercase tracking-widest text-[var(--text-tertiary)]">Cloud Sync</span>
                                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-[var(--bg-tertiary)] border-b border-[var(--border-primary)]">
                                ...
                                <th className="px-3 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-tertiary)] w-[18%]">Lead / Prospect</th>
                                ...
                                <th className="px-3 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-tertiary)] w-[8%] text-center">Info</th>
                                    ...
                                    return (
                                        <tr key={lead._id} className="hover:bg-[var(--bg-tertiary)]/30 transition-colors group">
                                            ...
                                            <td className="px-3 py-3 align-middle">
                                                <div className="flex flex-col gap-1 min-w-0">
                                                    {lead.emails?.map((email, i) => (
                                                        <div key={i} className="flex items-center justify-between group/item px-2 py-0.5 bg-blue-500/5 rounded-md border border-blue-500/10 hover:border-blue-500/30 transition-all">
                                                            <span className="text-[9px] font-bold text-[var(--text-secondary)] truncate mr-2">{email.value}</span>
                                                            <button
                                                                onClick={() => handleCopy(email.value, `e-${lead._id}-${i}`)}
                                                                className="opacity-0 group-hover/item:opacity-100 transition-opacity p-0.5 hover:bg-blue-500/10 rounded flex items-center justify-center shrink-0"
                                                            >
                                                                {copiedId === `e-${lead._id}-${i}` ? (
                                                                    <span className="text-[7px] font-black text-emerald-500">OK</span>
                                                                ) : (
                                                                    <svg className="w-2.5 h-2.5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                                                                )}
                                                            </button>
                                                        </div>
                                                    ))}
                                            ...
                                            <td className="px-3 py-3 align-middle">
                                                <div className="flex flex-col gap-1 min-w-0">
                                                    {lead.phones?.map((phone, i) => (
                                                        <div key={i} className="flex items-center justify-between group/item px-2 py-0.5 bg-green-500/5 rounded-md border border-green-500/10 hover:border-green-500/30 transition-all">
                                                            <span className="text-[9px] font-bold text-[var(--text-secondary)] truncate mr-2">{phone}</span>
                                                            <button
                                                                onClick={() => handleCopy(phone, `p-${lead._id}-${i}`)}
                                                                className="opacity-0 group-hover/item:opacity-100 transition-opacity p-0.5 hover:bg-green-500/10 rounded flex items-center justify-center shrink-0"
                                                            >
                                                                {copiedId === `p-${lead._id}-${i}` ? (
                                                                    <span className="text-[7px] font-black text-emerald-500">OK</span>
                                                                ) : (
                                                                    <svg className="w-2.5 h-2.5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                                                                )}
                                                            </button>
                                                        </div>
                                                    ))}
                                            ...
                                            <td className="px-3 py-3 align-middle text-center">
                                                <button
                                                    onClick={() => setReasonModal({ show: true, text: reason })}
                                            ...
                                            <td className="px-3 py-3 align-middle">
                                                <div className="flex items-center gap-1.5">
                                                    <div className="w-5 h-5 rounded bg-emerald-500/5 flex items-center justify-center text-[8px] font-black text-emerald-500 border border-emerald-500/10">
                                                        {lead.rejectionRequestedBy?.name?.[0] || 'M'}
                                                    </div>
                                                    <div className="flex flex-col min-w-0">
                                                        <span className="text-[9px] font-black truncate">{lead.rejectionRequestedBy?.name || 'Manager'}</span>
                                                        <span className="text-[7px] font-bold text-[var(--text-tertiary)] uppercase opacity-50 truncate">
            ...
            {/* Reason Modal - Ultra Compact Justification */}
            {reasonModal.show && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[120] p-4 animate-fadeIn">
                    <div className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-[20px] w-full max-w-[280px] overflow-hidden shadow-2xl animate-scaleIn">
                        <div className="p-3.5 space-y-3">
                            <div className="flex items-center gap-2.5 pb-2 border-b border-[var(--border-primary)]/50">
                                <div className="p-1 bg-emerald-500/10 rounded-lg text-emerald-500">
                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                </div>
                                <h3 className="text-[9px] font-black text-[var(--text-primary)] uppercase tracking-widest">Justification</h3>
                            </div>
                            <div className="bg-[var(--bg-tertiary)]/30 border border-[var(--border-primary)]/20 p-3 rounded-lg">
                                <p className="text-[9px] font-bold text-[var(--text-secondary)] italic leading-relaxed text-center">
                                    "{reasonModal.text}"
                                </p>
                            </div>
                            <button
